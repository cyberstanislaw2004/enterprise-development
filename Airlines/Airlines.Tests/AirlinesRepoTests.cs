...
public class AirlinesRepoTests(AirlinesRepoFixture fixture) : IClassFixture<AirlinesRepoFixture>
{
    ...
    {
        var flights = fixture.FlightService.GetFlights();
        var tickets = fixture.TicketService.GetTickets();
    ...
    public void GetFlightsWithMinDuration_WhenFlightsExist_ReturnsAllWithMinDuration()
    {
        ...
        var minDuration = (
            from flight in flights
            where flight.Duration == flights.Min(f => f.Duration)
            select new
            {
                Flight = flight.FlightNumber,
        ...
        var expectedMinDuration = flights.Min(f => f.Duration);
    ...
    public void GetPassengersWithZeroBaggage_OnSelectedFlight_ReturnsPassengersOrderedByFullName()
    {
        ...
        var selectedFlight = flights.First(f => f.FlightNumber == "SU100");
        ...
        {
            Assert.Equal(selectedFlight.FlightNumber, tickets.First(t => t.PassengerInfo.Id == passenger.Id).FlightInfo.FlightNumber);
            Assert.Equal(0, tickets.First(t => t.PassengerInfo.Id == passenger.Id).TotalBaggageWeight);
        }
